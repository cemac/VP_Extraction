...
Includes correction for PBB (using a standard DEM file).
...
from VP.utilities import preprocessing
...
def main():
    ...
    # ADD PBB and SNRH field to all VPs
    
    for VP in vps:
        VP.add_new_field('SNRH', 'Signal to noise ratio (H)', 'SNR', 'dB')
        VP.add_new_field('PBB','Partial beam blockage fraction estimate from DTM', 'PBB estimate', '-')
    ...
    for f, file_ in enumerate(file_list):
        ...
        noise_est, snrh = preprocessing.estimate_noise_level(radar)
        radar.add_field('SNRH', dic={'data':snrh, 'standard_name': 'SNR estimate', 'long_name': '', 'units': 'dB'}, replace_existing=True)
        gatefilter = pyart.filters.GateFilter(radar)
        gatefilter = pyart.correct.despeckle_field(radar,'SNRH',threshold=-2,size=25,)
        gatefilter.exclude_below('SNRH',-2)
        gatefilter.include_above('SNRH',10)
        gatefilter.include_gates(spnd.median_filter(gatefilter.gate_included,5),op='new')

        # Calculate PBB (under dev)
